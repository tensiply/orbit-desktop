...
const INSTALL_OPTIONS: { method: InstallMethod; label: string; hint: string; command: string }[] = [
  {
    ...
    hint:    'Pre-built binary for your platform — fastest.',
...
export function CliStep() {
  ...
  const installed = cliInfo?.installed ?? false
  const hasUpdate = updateCheck?.cli.has_update ?? false
  const latestVer = updateCheck?.cli.latest ?? null
  const isLoading = cliInfo === null

  const recheck = async () => {
    setChecking(true)
    await Promise.all([checkCli(), checkSetup(), checkUpdates()])
    setChecking(false)
  }
  ...
  const runInstall = async () => {
    ...
    const unlisten = await listen<string>('cli_install_output', (ev) => {
      setRunLines((prev) => [...prev, ev.payload])
    })
    try {
  ...
  const selectedOpt = INSTALL_OPTIONS.find((o) => o.method === method)!
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {cliInfo?.version && (
              <code className="text-xs font-mono text-muted-foreground">{cliInfo.version}</code>
            )}
            {hasUpdate && latestVer && (
        ...
        {cliInfo?.path && (
          <div className="flex items-center gap-2 rounded-md bg-muted/40 px-3 py-2">
            ...
            <code className="text-[11px] font-mono text-muted-foreground break-all">{cliInfo.path}</code>
          </div>
        )}

        {/* Update banner */}
        {hasUpdate && latestVer && !runDone && (
          <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-3">
            <p className="text-sm font-medium">Update available — {latestVer}</p>
              ...
              {INSTALL_OPTIONS.map((opt) => (
                ...
                >
                  <div className={cn(
                    ...
                    method === opt.method ? 'border-primary' : 'border-muted-foreground/30',
                  )}>
                    {method === opt.method && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                  </div>
                  <div>
            ...
            <code className="block text-[11px] font-mono text-muted-foreground bg-muted/50 rounded px-2 py-1.5 break-all">
              {selectedOpt.command}
            </code>
            {(running || runError) && (
              <>
                <ConsoleOutput lines={runLines} running={running} />
                {runError && (
            ...
            >
              {running
                ...
                : runError ? 'Retry'
                : <><ArrowUpCircle size={13} />Update to {latestVer}</>
              }
            </Button>
          </div>
        )}
        ...
        {runDone && (
          <div className="flex items-center gap-2 rounded-md bg-primary/8 px-3 py-2">
            ...
            <span className="text-sm text-primary font-medium">
              {hasUpdate ? 'Updated successfully' : 'Installed successfully'}
            </span>
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-xs text-muted-foreground font-medium">Choose an install method:</p>
        {INSTALL_OPTIONS.map((opt) => (
          <button
            key={opt.method}
            onClick={() => setMethod(opt.method)}
      ...
      <div className="flex items-center gap-2 rounded-md bg-muted/40 px-3 py-2">
        <Terminal size={12} className="text-muted-foreground/50 shrink-0" />
        <code className="text-[11px] font-mono text-muted-foreground break-all">{selectedOpt.command}</code>
      </div>
      ...
      {runDone && (
        <div className="flex items-center gap-2 rounded-md bg-primary/8 px-3 py-2">
          <CheckCircle2 size={14} className="text-primary shrink-0" />
          <span className="text-sm text-primary font-medium">Installed successfully</span>
        </div>
      )}
